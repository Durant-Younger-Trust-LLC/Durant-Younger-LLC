...
type RestOf<T extends IBankTransaction | NewBankTransaction> = Omit<T, 'date'> & {
  date?: string | null;
};
...
export class BankTransactionService {
  ...
  addBankTransactionToCollectionIfMissing<Type extends Pick<IBankTransaction, 'id'>>(
    ...
    if (bankTransactions.length > 0) {
      const bankTransactionCollectionIdentifiers = bankTransactionCollection.map(
        bankTransactionItem => this.getBankTransactionIdentifier(bankTransactionItem)!
      );
  ...
  protected convertDateFromClient<T extends IBankTransaction | NewBankTransaction | PartialUpdateBankTransaction>(
    ...
    return {
      ...bankTransaction,
      date: bankTransaction.date?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBankTransaction: RestBankTransaction): IBankTransaction {
    return {
      ...restBankTransaction,
      date: restBankTransaction.date ? dayjs(restBankTransaction.date) : undefined,
    };
  }
