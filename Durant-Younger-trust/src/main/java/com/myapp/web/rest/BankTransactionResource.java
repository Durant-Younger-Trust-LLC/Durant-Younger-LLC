...
/**
 * REST controller for managing {@link com.myapp.domain.BankTransaction}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<BankTransaction> result = bankTransactionRepository
            ...
            .map(existingBankTransaction -> {
                if (bankTransaction.getDate() != null) {
                    existingBankTransaction.setDate(bankTransaction.getDate());
                }
                if (bankTransaction.getDescription() != null) {
                    existingBankTransaction.setDescription(bankTransaction.getDescription());
                }
                if (bankTransaction.getBalance() != null) {
                    existingBankTransaction.setBalance(bankTransaction.getBalance());
                }
                if (bankTransaction.getMerchantName() != null) {
                    existingBankTransaction.setMerchantName(bankTransaction.getMerchantName());
                }
